...
const handleSaveUpiEdit = async () => {
  ...
  const handleChangePassword = async (e) => {
    ...
    try {
      if (!userId) {
        setMessage1("User ID is missing");
        return;
      }
  
      if (!token) {
        setMessage1("Authentication token is missing");
        return;
      }
      ...
      setMessage1(response.data.message); // Show success message
    } catch (error) {
      setMessage1(error.response?.data?.message || "Something went wrong");
    }
  };
...
>
  {isEditUpiMode ? "Save" : (upi && upi.length > 0 ? "Edit" : "Add Details")}
      ...
      {message1 && <p className="col-span-2 text-center text-red-500">{message1}</p>}
